...
export const LIGHT_ROLES = {
  ...
  secondary:  { source: 'accent',  shade: '200' },  // neutral/200 in single scheme
};
...
export const DARK_ROLES = {
  ...
  secondary:    { source: 'accent',  shade: '700' },
};

export function resolveTokens(colorScale, neutralScale, secondaryScale, dark) {
  const roles = dark ? DARK_ROLES : LIGHT_ROLES;
  // single scheme: no accent → secondary falls back to neutral (visually distinct from primary)
  const accentScale = secondaryScale || neutralScale;
  const tokens = {};
